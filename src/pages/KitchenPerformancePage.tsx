...
export function KitchenPerformancePage() {
  ...
  const kitchenCoverageNote = report ? (report.kitchen_completion_source === "durable" ? "Completion uses durable ready and delivered states." : "Fallback completion measures automation queue progression.") : "Kitchen completion source is unavailable.";
  ...
  return <section className="kitchen-performance-workspace">
    ...
    <div className="kitchen-performance-grid">
      ...
      <Panel label="Kitchen telemetry"><PanelHeading eyebrow="Operational analytics" title="Data coverage" action={<Printer size={20} aria-hidden="true" />} /><div className="performance-list"><div><span>Recorded orders</span><strong>{report?.orders.available ? report.orders.value : "Unavailable"}</strong></div><div><span>Kitchen completion</span><strong>{report?.kitchen_completion.available ? `${report.kitchen_completion.value.toFixed(0)}%` : "Unavailable"}</strong></div><div><span>Printer availability</span><strong>{report?.printer_availability.available ? `${report.printer_availability.value.toFixed(0)}%` : "Unavailable"}</strong></div></div><p className="automation-footnote">{kitchenCoverageNote} Cycle-time reporting requires persisted kitchen stage timestamps.</p></Panel>
    </div>
