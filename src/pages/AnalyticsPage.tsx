  ...
  return (
    <section className="analytics-workspace">
      ...
      <div className="analytics-grid">
        ...
        <Panel label="Operational performance"><PanelHeading eyebrow="Operational data" title="Key performance" /><div className="performance-list"><div><span>Printer utilization</span><strong>{metricValue(report?.printer_utilization, "%")}</strong><small>{report?.printer_utilization.available ? `${report.printer_utilization.printed} printed · ${report.printer_utilization.failed} failed` : "Runtime telemetry required"}</small></div><div><span>Delivery time</span><strong>{metricValue(report?.delivery_time)}</strong></div><div><span>Staff task completion</span><strong>{metricValue(report?.staff_productivity, "%")}</strong><small>{report?.staff_productivity.available ? `${report.staff_productivity.completed_tasks} of ${report.staff_productivity.total_tasks} tasks` : "Task history required"}</small></div><div><span>Average ticket</span><strong>{moneyValue(report?.average_ticket)}</strong></div></div></Panel>
      </div>

      <Panel label="Analytics data quality"><PanelHeading eyebrow="Data coverage" title="Operational reporting" action={<Flame size={20} aria-hidden="true" />} /><p className="analytics-summary-copy">Order events, printer telemetry, durable order totals, delivery updates, and shift-task completion records feed this report. Metrics remain unavailable when their required source records do not exist.</p></Panel>
    </section>
  );
}
