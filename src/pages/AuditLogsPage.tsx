...
import { listAuditLogs, listRoleAssignmentEvents } from "@/features/audit/api";
...
export function AuditLogsPage() {
  ...
  const accessAuditQuery = useQuery({ queryKey: ["audit", "role-assignments"], queryFn: listRoleAssignmentEvents, refetchInterval: 10_000 });
  ...
  return <section className="audit-logs-workspace">
    <div className="automation-toolbar">
      ...
      <button type="button" className="icon-button" onClick={() => { logsQuery.refetch(); accessAuditQuery.refetch(); }} disabled={logsQuery.isFetching || accessAuditQuery.isFetching} title="Refresh audit logs" aria-label="Refresh audit logs"><RefreshCw size={18} aria-hidden="true" className={logsQuery.isFetching || accessAuditQuery.isFetching ? "spin" : undefined} /></button>
    </div>
    ...
    {accessAuditQuery.isError && <div className="orders-api-error" role="alert"><span>{accessAuditQuery.error.message}</span><button type="button" className="text-button" onClick={() => accessAuditQuery.refetch()}>Retry</button></div>}
    ...
    <Panel label="Access audit history"><PanelHeading eyebrow="Role changes" title="Access assignments" action={<span className="task-count">{accessAuditQuery.data ? `${accessAuditQuery.data.length} changes` : "Unavailable"}</span>} /><div className="access-audit-list">{accessAuditQuery.isPending && <EmptyState message="Loading access changes..." />}{accessAuditQuery.data?.map((event) => <article className="access-audit-row" key={event.id}><time>{new Date(event.created_at).toLocaleString([], { dateStyle: "medium", timeStyle: "medium" })}</time><div><strong>{event.actor_email}</strong><span>Changed {event.target_subject} from {event.previous_role_name || "Unknown"} to {event.new_role_name || "Unknown"}</span></div></article>)}{!accessAuditQuery.isPending && !accessAuditQuery.isError && accessAuditQuery.data?.length === 0 && <EmptyState message="No role assignments have been recorded." />}</div></Panel>
  </section>;
}
