...
function eventMetadata(event: { attempt?: number; payload?: Record<string, string> }) {
  return [event.payload?.destination, event.attempt ? `Attempt ${event.attempt}` : undefined].filter(Boolean).join(" · ");
}
