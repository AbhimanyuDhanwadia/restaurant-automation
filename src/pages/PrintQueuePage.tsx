...
import { listPrintJobs, reprintOrder, retryPrintJob, type PrintJobStatus } from "@/features/printqueue/api";
...
export function PrintQueuePage() {
  ...
  return <section className="print-queue-workspace">
    ...
    {(jobsQuery.isError || reprintMutation.isError || retryMutation.isError) && <div className="orders-api-error" role="alert"><span>{jobsQuery.error?.message ?? reprintMutation.error?.message ?? retryMutation.error?.message}</span><button type="button" className="text-button" onClick={() => jobsQuery.refetch()}>Retry</button></div>}
