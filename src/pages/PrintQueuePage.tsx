...
export function PrintQueuePage() {
  ...
  const requeueMutation = useMutation({ mutationFn: requeuePrintJob, onSuccess: () => queryClient.invalidateQueries({ queryKey: ["printers", "queue"] }) });
  const retryMutation = useMutation({ mutationFn: retryPrintJob, onSuccess: () => queryClient.invalidateQueries({ queryKey: ["printers", "queue"] }) });
  ...
  return <section className="print-queue-workspace">
    ...
    {(jobsQuery.isError || reprintMutation.isError || requeueMutation.isError || retryMutation.isError) && <div className="orders-api-error" role="alert"><span>{jobsQuery.error?.message ?? reprintMutation.error?.message ?? requeueMutation.error?.message ?? retryMutation.error?.message}</span><button type="button" className="text-button" onClick={() => jobsQuery.refetch()}>Retry</button></div>}
