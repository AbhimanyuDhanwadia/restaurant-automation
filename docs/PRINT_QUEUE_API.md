...
- `POST /api/v1/printers/queue/{jobID}/requeue` atomically marks a job currently in `printing` as `reviewed`, creates a new durable ticket from it, and submits the replacement after operator review.
...
Migration `0009_print_jobs.sql` stores ticket lines, status, attempts, errors, and timestamps. Jobs progress through `queued`, `printing`, `printed`, or `failed`; printer lifecycle callbacks update the record without blocking physical printing. Migration `0018_reviewed_print_jobs.sql` adds the terminal `reviewed` state used when an operator resolves an indeterminate printing job by requeueing it.
...
At server startup, queued jobs are atomically claimed before they are re-dispatched after printer workers and observers are ready. This prevents multiple API instances from replaying the same durable ticket during concurrent startup. Jobs already marked `printing` are not automatically replayed because their physical outcome is indeterminate after a restart; they are surfaced as printer-anomaly insights and can be requeued by job ID after operator review. The source job becomes `reviewed` in the same transaction that creates its queued replacement, so it cannot be requeued repeatedly or left looking active. Failed jobs can be retried by job ID, while order reprints still create a new ticket from the latest durable ticket for that order.
