...
`GET /api/v1/admin/roles` lists roles. `POST /api/v1/admin/roles` creates a custom role. Role names must be unique and at least one supported permission is required. This milestone does not assign roles to users.
